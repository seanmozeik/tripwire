...
- Clarified documentation for the `rg` command to correctly indicate that `-r` is the `--replace` flag, not a recursive search option.

## [0.6.4] - 2026-06-07

## Fixed

- The application now fails loudly when the configuration is broken, instead of silently defaulting to incorrect values.

## [0.6.3] - 2026-06-07

## Fixed

- Fixed bash script parsing to correctly split newline statements while preserving quoting behavior.
...
- Resolved tripwire-hook path resolution at runtime for script and bundled CLI modes, ensuring proper functionality in both execution contexts.

## [0.5.2] - 2026-05-26

## Changed

- CLI now dynamically resolves the dispatch bin path for better compatibility and flexibility.

## [0.5.1] - 2026-05-24

## Fixed

- Fixed bash heredoc content to properly handle bypass markers, preventing them from being hidden in the output.
...
_Generated automatically based on release commits and context._
_(Note: The above is a comprehensive example. For your actual submission, I will provide only the concise, user-facing sections as requested.)_
