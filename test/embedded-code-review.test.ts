...
bunTest.describe('adversarial review regressions (policy-only)', () => {
  bunTest.test.each([
    ...
    javascript('require("fs").writeFileSync(".env", "x", "hex")'),
    javascript('require("fs").writeFileSync(".env", "x", {encoding:"base64"})'),
    ...
    'uv --project /tmp run -- - <<\'PY\'\nimport os; os.unlink("/protected")\nPY',
    'env uv run --no-project - <<\'PY\'\nimport os; os.unlink("/protected")\nPY',
    'printf \'import os; os.unlink("/protected")\' | uv run -',
    'uv run --script=- <<\'PY\'\nimport os; os.unlink("/protected")\nPY',
    'uv run --script=/dev/stdin <<\'PY\'\nimport os; os.unlink("/protected")\nPY',
