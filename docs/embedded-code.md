...
`bun run verify` checks formatting, lint, types, the complete test suite, and both build outputs. The build sends all 355 corpus fixtures through each packaged hook: 139 original cases, 114 compatibility and adversarial cases, 48 command-classification cases, and 54 write-policy cases. Write-only negative controls now use protected destinations; ordinary destinations have paired positive cases. Explicit deletion and truncation controls remain unchanged. The build also checks manifest-backed package scripts. The Pi/OMP tests pass normalized events through the production hook and assert an actual embedded-code denial.
