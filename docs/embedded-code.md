...
`bun run verify` checks formatting, lint, types, the complete test suite, and both build outputs. The build sends all 253 corpus fixtures through each packaged hook: the 139 original cases plus 114 compatibility and adversarial cases. The latter contain 33 allow/block pairs and 48 extra attacks. The adversarial review suite adds option, alias, path, resource, and runtime-context cases. The Pi/OMP tests pass normalized events through the production hook and assert an actual embedded-code denial.
...
The full verification run passed 703 tests and all 253 fixtures through each of the native and portable packaged hooks. The build ran in a temporary copy and did not replace the installed hook. Strict review added checks for loop-carried aliases, JavaScript replacement tokens, regex backreferences, lossy decoding, write codecs, and shared-value inspection budgets.

Run `bun scripts/benchmark-code.ts` for the warm policy benchmark. One local run on Bun 1.4.2 produced 5,060 samples over 253 fixtures: median 0.033 ms, p95 0.083 ms, p99 0.136 ms, maximum 2.015 ms. These measurements include Bash analysis and filesystem policy checks; they exclude process startup and do not compare parser implementations. The benchmark verifies each decision against its fixture expectation.
