...
Directory destinations are checked both as submitted targets and as `<destination>/<basename(source)>` when the directory exists or the submitted destination ends with `/`. The shell transfer check also covers local `rsync` and `install`. Unquoted home-relative operands fail closed when shell home expansion cannot be proved, and `.ssh` itself is protected as well as its children. Unknown transfer options and remote rsync targets remain unverified.
